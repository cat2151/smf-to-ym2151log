...
def convert_to_ym2151_log(midi_events_data):
    ...
    for ch in range(8):
        ym2151_events.append({"time": 0, "addr": "0x08", "data": f"0x{ch:02X}"})
    ...
    for event in midi_events:
        ...
        if event["type"] == "tempo":
            ...
            sample_time = ticks_to_samples(event["ticks"], ticks_per_beat, current_tempo_bpm)

            note = event["note"]
            kc, kf = midi_to_kc_kf(note)

            # Set KC (Key Code)
            ym2151_events.append({"time": sample_time, "addr": f"0x{0x28 + ym2151_channel:02X}", "data": f"0x{kc:02X}"})

            # Set KF (Key Fraction)
            ym2151_events.append({"time": sample_time, "addr": f"0x{0x30 + ym2151_channel:02X}", "data": f"0x{kf:02X}"})

            # Key ON (0x78 = all operators on)
            ym2151_events.append({"time": sample_time, "addr": "0x08", "data": f"0x{0x78 | ym2151_channel:02X}"})
            ...
            if note in active_notes:
                # Key OFF
                ym2151_events.append({"time": sample_time, "addr": "0x08", "data": f"0x{ym2151_channel:02X}"})

                del active_notes[note]

    return {"event_count": len(ym2151_events), "events": ym2151_events}
